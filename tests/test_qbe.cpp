...
auto gen_ast_qbe(std::string source) -> json {
    yal::MemStream ms;
    ...
    yal::codegen::qbe::codegen(ms.f, module, er, ts, {.single_file = true});
    ms.flush();
...
auto gen_ast_qbe_many(std::vector<std::string> sources) -> json {
    yal::MemStream ms;
    ...
    yal::codegen::qbe::codegen(ms.f, module, er, ts, {});
    ms.flush();
...
static void run_test(Context& ctx, TestParams const& p, std::string name,
    ...
    run_checks_for_test(ctx, p, name, skip,
                        [&]() { return gen_ast_qbe(source); });
}

static void run_test(Context& ctx, TestParams const& p, std::string name,
                     std::vector<std::string> source, bool skip = false) {
    run_checks_for_test(ctx, p, name, skip,
                        [&]() { return gen_ast_qbe_many(source); });
}

auto test_qbe(TestParams const& p) -> std::tuple<int, int, int> {
    Context ctx{.tags = {"qbe"}, .filters = p.filters, .tests_ran = {}};
